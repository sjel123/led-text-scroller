...
- `USER_GUIDE.md` contains a deeper walkthrough of `app.py`, the worker thread, UI controls, and the quote generator, plus a Mermaid flowchart showing how quotes flow through the system.
- `daily_quote_generator.py` now wraps the OpenAI chat API (with local fallbacks) and is wired to `/daily-quote` so the UI can get deterministic and refreshed quotes.
